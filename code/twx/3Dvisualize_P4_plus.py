...
def visualize_team_strategy(team_strategies, n_points=1000, threshold_percent=70.0):
    ...
    print(f"--- 开始生成 {num_uavs} 架无人机协同策略的高精度可视化动画 ---")
    ...
    for uav_name in uav_names:
        ...
        t_drop, t_delay = strategy['drops'][0]
    ...
    individual_occlusion_times = [0.0] * num_uavs
    ...
    for i, t in enumerate(times):
        ...
        for j in range(num_uavs): 
            smoke_pos = all_smoke_cloud_trajs[j][i]
            occluded_count_for_this_uav = 0
            if not np.any(np.isnan(smoke_pos)):
                for point_idx, tp in enumerate(target_points):
                    if is_line_segment_intersecting_sphere_numba(missile_pos, tp, smoke_pos, SMOKE_RADIUS):
                        occluded_count_for_this_uav += 1
                        total_occluded_indices_this_step.add(point_idx) # 加入协同遮蔽集合
            
            if occluded_count_for_this_uav >= required_occluded_count:
    ...
    for i, uav_name in enumerate(uav_names):
        print(f"  {uav_name}: {individual_occlusion_times[i]:.2f} 秒")
    ...
    for uav_name in uav_names:
        ...
        all_grenade_trajs_vis.append(g_traj)
    ...
    for i, t in enumerate(times):
        ...
        frame_data.append(go.Scatter3d(x=[trajectory['M1'][i][0], TRUE_TARGET_BASE_CENTER[0]], y=[trajectory['M1'][i][1], TRUE_TARGET_BASE_CENTER[1]], z=[trajectory['M1'][i][2], TRUE_TARGET_HEIGHT/2], line=dict(color=los_color)))

        for j in range(num_uavs):
            frame_data.append(go.Scatter3d(x=[all_grenade_trajs_vis[j][i][0]], y=[all_grenade_trajs_vis[j][i][1]], z=[all_grenade_trajs_vis[j][i][2]]))
            frame_data.append(go.Scatter3d(x=[all_smoke_cloud_trajs[j][i][0]], y=[all_smoke_cloud_trajs[j][i][1]], z=[all_smoke_cloud_trajs[j][i][2]]))
            
        individual_times_str = " | ".join([f"{uav_names[k]}: {cumulative_individual_times_history[i][k]:.2f}s" for k in range(num_uavs)])
