...
def get_occlusion_timeline(team_strategies, uav_pos_dict, target_point, time_step=0.1):
    ...
    for uav_name, strat in team_strategies.items():
        ...
        if 't_drop' in strat: drops = [(strat['t_drop'], strat['t_delay'])]
        for drop in drops:
    ...
    for t_explode in search_explode_times:
        for t_delay in search_delays:
            ...
            for los_ratio in search_los_ratios:
                ...
                required_vel_xy = required_vel_3d[:2]; required_speed = np.linalg.norm(required_vel_xy)
...
def stage1_individual_coarse_search_parallel(n_top):
    ...
    tasks = [(name, UAV_INITIAL_POS[name], n_top) for name in UAV_NAMES]; individual_strategies = {}
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        results = list(tqdm(pool.imap_unordered(stage1_worker, tasks), total=len(tasks), desc="全局单弹策略粗搜"))
    for uav_name, strategies in results: individual_strategies[uav_name] = strategies
...
def stage2_team_formation_and_expansion(individual_strats, n_teams):
    ...
    for team in top_single_grenade_teams:
        ...
        for uav_name, strat in team.items():
            t_drop_center = strat['t_drop']; base_delay = strat['t_delay']
            expanded_strat = {'v': strat['v'], 'theta_rad': strat['theta_rad'], 'drop1': (t_drop_center - DROP_INTERVAL, base_delay), 'drop2': (t_drop_center, base_delay), 'drop3': (t_drop_center + DROP_INTERVAL, base_delay)}
            if expanded_strat['drop1'][0] < 0.1:
                expanded_strat['drop1'] = (0.1, base_delay); expanded_strat['drop2'] = (0.1 + DROP_INTERVAL, base_delay); expanded_strat['drop3'] = (0.1 + 2 * DROP_INTERVAL, base_delay)
    ...
    for i in range(NUM_OPTIMIZATION_ROUNDS):
        ...
        for uav_name in UAV_NAMES:
            for key in ['v', 'theta_rad']:
                original_val = params[uav_name][key]; step = 2.0 if key == 'v' else np.radians(4)
                ...
                for test_val in search_range:
                    ...
                    if t > current_max_time: current_max_time = t; params[uav_name][key] = test_val
            for j in range(1, 4):
                drop_key = f'drop{j}'; t_drop_base, t_delay_base = params[uav_name][drop_key]
                for t_drop_test in np.linspace(max(0.1, t_drop_base - 0.5), t_drop_base + 0.5, 5):
                    test_params = params.copy(); test_params[uav_name] = test_params[uav_name].copy(); test_params[uav_name][drop_key] = (t_drop_test, t_delay_base)
                    t = get_occlusion_timeline(test_params, UAV_INITIAL_POS, simple_target_point)
                    if t > current_max_time: current_max_time = t; params[uav_name][drop_key] = (t_drop_test, t_delay_base)
                ...
                for t_delay_test in np.linspace(max(0.1, t_delay_base - 0.5), t_delay_base + 0.5, 5):
                    test_params = params.copy(); test_params[uav_name] = test_params[uav_name].copy(); test_params[uav_name][drop_key] = (t_drop_base, t_delay_test)
                    t = get_occlusion_timeline(test_params, UAV_INITIAL_POS, simple_target_point)
                    if t > current_max_time: current_max_time = t; params[uav_name][drop_key] = (t_drop_base, t_delay_test)
...
def stage4_final_validation_team(final_team, n_points=1000, threshold_percent=70.0):
    ...
    target_points = np.array(target_points); smoke_events, final_team_details = [], {}; min_explode_time, max_end_time = float('inf'), float('-inf')
    for uav_name in UAV_NAMES:
        strat = final_team[uav_name]; v, theta_rad = strat['v'], strat['theta_rad']
        uav_velocity = np.array([np.cos(theta_rad)*v, np.sin(theta_rad)*v, 0])
        final_team_details[uav_name] = {'v': v, 'theta_rad': theta_rad, 'drops': []}
        for i in range(1, 4):
            t_drop, t_delay = strat[f'drop{i}']; t_explode = t_drop + t_delay
...
def save_initial_teams_to_csv(initial_teams, filename="stage2_initial_teams_2UAV_6G.csv"):
    ...
    for name in UAV_NAMES:
        header.extend([f'{name}_v', f'{name}_theta_deg', 
...
def save_optimization_process_to_csv(optimized_teams, initial_teams, histories, filename="stage3_optimization_process_2UAV_6G.csv"):
    ...
    for name in UAV_NAMES:
        base_header.extend([f'{name}_v', f'{name}_theta_deg', f'{name}_t_drop1', f'{name}_t_delay1', f'{name}_t_drop2', f'{name}_t_delay2', f'{name}_t_drop3', f'{name}_t_delay3'])
    ...
    with open(filename, 'w', newline='') as f:
        ...
        for i, (opt_team, init_team, history) in enumerate(zip(optimized_teams, initial_teams, histories)):
            ...
            for name in UAV_NAMES:
                strat = opt_team[name]
                row.extend([f"{strat['v']:.4f}", f"{np.degrees(strat['theta_rad']):.4f}", f"{strat['drop1'][0]:.4f}", f"{strat['drop1'][1]:.4f}", f"{strat['drop2'][0]:.4f}", f"{strat['drop2'][1]:.4f}", f"{strat['drop3'][0]:.4f}", f"{strat['drop3'][1]:.4f}"])
...
def visualize_optimization_journey(initial_teams, optimized_teams):
    ...
    def team_to_vector(team):
        ...
        for name in UAV_NAMES:
            strat = team[name]
            vec.extend([strat['v'], strat['theta_rad'], strat['drop1'][0], strat['drop1'][1], strat['drop2'][0], strat['drop2'][1], strat['drop3'][0], strat['drop3'][1]])
        return vec
...
if __name__ == "__main__":
    ...
    if not initial_teams:
        ...
        for uav_name in UAV_NAMES:
            ...
            print(f"\n--- 无人机 {uav_name} 统一飞行策略 ---")
            print(f"  飞行速度 (v)      : {details['v']:.4f} m/s"); print(f"  飞行方向 (θ)      : {np.degrees(details['theta_rad']):.4f} 度")
            for i, drop_info in enumerate(details['drops']):
                print(f"\n  --- {uav_name} 第 {i+1} 枚烟幕弹 ---")
                print(f"    投放时间 (t_drop)   : {drop_info['t_drop']:.4f} 秒"); print(f"    引信延迟 (t_delay)    : {drop_info['t_delay']:.4f} 秒")
        ...
        print("\n" + "-"*70); print("--- 最终性能评估 ---"); print(f"  遮蔽有效性阈值          : {OCCLUSION_THRESHOLD_PERCENT:.1f}%"); print(f"  最终高精度总有效遮蔽时长: {final_time:.4f} 秒"); print("="*70)
