...
def step2_local_optimization_multi(initial_params):
    ...
    if t_drop_early < 0.1: t_drop_early = 0.1; t_drop_center = t_drop_early + DROP_INTERVAL; t_drop_late = t_drop_center + DROP_INTERVAL
    ...
    for i in range(OPTIMIZATION_ITERATIONS):
        ...
        for theta_test in np.linspace(params['theta_rad']-np.radians(10), params['theta_rad']+np.radians(10), 11):
            test_params = params.copy(); test_params['theta_rad'] = theta_test; t = calculate_simple_total_occlusion_time(test_params)
...
def step3_final_validation_multi(final_params, n_points=1000, threshold_percent=70.0):
    ...
    uav_velocity = np.array([np.cos(theta_rad) * v, np.sin(theta_rad) * v, 0]); smoke_events = []
    for i in range(1, 4):
        t_drop, t_delay = final_params[f'drop{i}']; t_explode = t_drop + t_delay
        p_drop = uav_initial_pos + uav_velocity * t_drop
        p_explode = p_drop + uav_velocity * t_delay + np.array([0, 0, -0.5 * GRAVITY * t_delay**2])
    ...
    min_explode_time = min(se['t_explode'] for se in smoke_events); max_end_time = max(se['t_explode'] for se in smoke_events) + SMOKE_DURATION
    for t in np.arange(min_explode_time, max_end_time, time_step):
...
def save_results_to_csv(results_list, initial_list, histories_list, filename="optimization_results.csv"):
    ...
    iter_header = [f'iter_{i+1}_score' for i in range(OPTIMIZATION_ITERATIONS)]
    ...
    with open(filename, 'w', newline='') as f:
        ...
        for i, (res, initial, history) in enumerate(zip(results_list, initial_list, histories_list)):
            ...
            row = [
                ...
                f"{res['drop1'][0]:.4f}", f"{res['drop1'][1]:.4f}",
                f"{res['drop2'][0]:.4f}", f"{res['drop2'][1]:.4f}",
                f"{res['drop3'][0]:.4f}", f"{res['drop3'][1]:.4f}",
            ]
...
def visualize_optimization_journey(initial_list, final_list):
    ...
    initial_vectors = np.array([[p['v'], np.degrees(p['theta_rad']), p['t_drop'], p['t_delay'], p['t_drop']+1.5, p['t_delay'], p['t_drop']+3.0, p['t_delay']] for p in initial_list])
...
if __name__ == "__main__":
    ...
    if not initial_solutions_list:
        ...
        print("\n--- 对全局最优候选解进行最终高精度验证 ---")
        final_time, final_params, smoke_events = step3_final_validation_multi(
