...
import numpy as np
from numba import njit # [新增] 导入numba

# ==============================================================================
# 1. Numba 加速的核心数学函数 (从优化脚本中引入)
# ==============================================================================
@njit(fastmath=True)
def is_line_segment_intersecting_sphere_numba(p1, p2, sphere_center, sphere_radius):
    ...
    t = max(0.0, min(1.0, t)) # 使用max/min代替clip以兼容numba
...
def visualize_team_strategy(team_strategies, n_points=100, threshold_percent=70.0):
    """
    根据给定的双机协同策略，生成并显示战场态势的动态模拟动画。
    [修改] 使用高精度的多点百分比遮蔽判断。

    参数:
    team_strategies (dict): 包含多架无人机策略的字典。
    n_points (int): 用于验证遮蔽的目标采样点数量。
    threshold_percent (float): 判定为有效遮蔽所需的采样点被遮蔽的百分比。
    """
    ...
    for i, t in enumerate(times):
        ...
        occluded_points_indices = set() # 使用集合避免重复计数
        ...
        for smoke_traj in all_smoke_cloud_trajs:
            ...
            for point_idx, tp in enumerate(target_points):
                if point_idx in occluded_points_indices: continue # 如果这个点已经被其他云遮蔽，跳过
    ...
    num_dynamic_traces = 2 + 1 + 1 + 2*total_grenades # M1, 2xUAV, 2xLOS, 2x for each grenade
    ...
    for i, t in enumerate(times):
        ...
        frame_data = [
            ...
            go.Scatter3d(x=occluded_los_lines_x, y=occluded_los_lines_y, z=occluded_los_lines_z), # 被遮蔽的LOS
            go.Scatter3d(x=[missile_pos[0], true_target_center_point[0]], y=[missile_pos[1], true_target_center_point[1]], z=[missile_pos[2], true_target_center_point[2]], line=dict(color=los_color)) # 主LOS
        ]
        ...
        # 动态计算要更新的轨迹索引
        traces_to_update = list(range(animated_traces_start_index, len(fig.data) - 3)) # M1, UAVs, LOSs, Grenades, Smokes
